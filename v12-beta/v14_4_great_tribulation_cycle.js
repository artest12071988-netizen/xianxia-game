...
async function tick(){const sb=client(),cs=window.cloudState||get('cloudState');if(document.hidden||cs?.serviceRestricted||!sb||busy)return;busy=true;try{const {data,error}=await sb.rpc('great_tribulation_tick');if(error){console.warn('[FIX4D tick]',error.message);return;}const row=Array.isArray(data)?data[0]:data;if(row){if(window.cloudState)window.cloudState.worldControls=row;announce(row);updateBadge(row);}}finally{busy=false;}}
