...
(() => {
  ...
  const ITEM_EDITOR_BUILD='V15.4-ADMIN-BALANCE-PHASE1-FIX6-DIVINE-BEAST-EXTRA-DROPS-20260726';
  ...
  function normalizeBeastExtraDrops(beast){
    const rows=Array.isArray(beast?.extra_drops)?beast.extra_drops:[];
    return rows.map(x=>({item_id:String(x?.item_id||''),item_name:String(x?.item_name||itemName(x?.item_id)||''),drop_chance:n(x?.drop_chance),quantity:Math.max(1,Math.floor(n(x?.quantity)||1)),enabled:x?.enabled!==false})).filter(x=>x.item_id);
  }
  function renderBeastExtraDropRows(beast){
    const rows=normalizeBeastExtraDrops(beast);
    if(!rows.length)return '<div class="notice">目前沒有額外掉落物品；原有神獸專屬材料仍會照原規則掉落。</div>';
    return rows.map((d,i)=>{const exists=!!state.draft?.items?.[d.item_id];return `<div class="drop-row ${exists?'':'drop-invalid'}"><div><b>${h(exists?itemName(d.item_id):'不存在的物品 ID')}</b><div class="cfg-mini">ID ${h(d.item_id)}${exists?'':'｜儲存前必須移除'}</div></div><div><input type="number" min="0.01" max="100" step="0.01" value="${h(percentText(d.drop_chance))}" onchange="setDivineBeastDropRateV154('${h(beast.beast_key)}',${i},this.value)"><div class="cfg-mini">獨立掉落率 %</div></div><div><input type="number" min="1" max="9999" step="1" value="${h(d.quantity)}" onchange="setDivineBeastDropQuantityV154('${h(beast.beast_key)}',${i},this.value)"><div class="cfg-mini">數量</div></div><button class="btn red" onclick="removeDivineBeastDropV154('${h(beast.beast_key)}',${i})">移除</button></div>`}).join('');
  }
  function beastDropItemOptions(beast,search=''){
    const q=String(search||'').trim().toLowerCase(),fixed=String(beast?.material_id||''),used=new Set(normalizeBeastExtraDrops(beast).map(x=>x.item_id));
    return Object.entries(state.draft?.items||{}).filter(([id,x])=>id!==fixed&&!used.has(String(id))&&(!q||(id+' '+(x.name||'')+' '+(x.cat||'')).toLowerCase().includes(q))).sort((a,b)=>String(a[0]).localeCompare(String(b[0]),'zh-Hant')).map(([id,x])=>`<option value="${h(id)}">${h(id)}｜${h(x.name||'')}｜${h(x.cat||'')}</option>`).join('');
  }
  function renderDivineBeastMonster(beast,options){
    ...
    const itemOptions=beastDropItemOptions(beast,'');
    return `<div class="phase1-scope"><b>一般怪物與五大神獸 BOSS 已整合在同一個搜尋清單。</b> 神獸仍使用原有獨立世界 BOSS 系統；原專屬材料掉落規則保留，額外掉落由本頁獨立設定。</div>
      <div class="monster-editor">
        ...
        <div class="beast-boss-note">目前選擇：五大神獸 BOSS｜${h(beast.beast_name)}。三圍與額外掉落寫回原有神獸資料，不建立重複怪物 ID，也不改神獸戰鬥方式。</div>
        ...
        <div class="beast-fixed-drop"><b>原有固定掉落（保留）：</b>${h(beast.material_name||itemName(beast.material_id))}（ID ${h(beast.material_id||'—')}）｜取得率 ${h(rewardPct)}%。<div class="cfg-mini">此專屬材料與原取得率不會被額外掉落設定覆蓋。</div></div>
        <div class="beast-extra-drop-editor"><h3 style="margin:0 0 8px">神獸額外掉落</h3><div class="drop-list">${renderBeastExtraDropRows(beast)}</div><div class="drop-add"><div class="field"><label>搜尋既有物品</label><input id="phase1BeastDropSearch" placeholder="輸入 ID 或名稱" oninput="filterDivineBeastDropItemsV154('${h(beast.beast_key)}',this.value)"></div><div class="field"><label>選擇物品</label><select id="phase1BeastDropItem">${itemOptions}</select></div><div class="field"><label>掉落率 %</label><input id="phase1BeastDropRate" type="number" min="0.01" max="100" step="0.01" value="10"></div><div class="field"><label>數量</label><input id="phase1BeastDropQuantity" type="number" min="1" max="9999" step="1" value="1"></div></div><div class="beast-extra-drop-actions"><button class="btn jade" onclick="addDivineBeastDropV154('${h(beast.beast_key)}')">加入額外掉落</button><button class="btn gold" onclick="saveDivineBeastExtraDropsV154('${h(beast.beast_key)}')">儲存額外掉落</button></div><div class="cfg-mini" style="margin-top:8px">每個額外物品獨立判定；只能選擇正式物品主檔中已存在的物品。</div></div>
  ...
  function currentBeastByKey(key){return (state.divineBeasts||[]).find(x=>String(x.beast_key)===String(key))||null}
  function filterDivineBeastDropItems(key,value){const beast=currentBeastByKey(key),select=document.getElementById('phase1BeastDropItem');if(beast&&select)select.innerHTML=beastDropItemOptions(beast,value)}
  function addDivineBeastDrop(key){
    const beast=currentBeastByKey(key),id=String(document.getElementById('phase1BeastDropItem')?.value||''),pct=Number(document.getElementById('phase1BeastDropRate')?.value),qty=Number(document.getElementById('phase1BeastDropQuantity')?.value);
    if(!beast||!state.draft?.items?.[id])return toast('請選擇有效的既有物品');
    if(id===String(beast.material_id||''))return toast('此物品已是神獸原有固定掉落，不可重複加入');
    if(!Number.isFinite(pct)||pct<=0||pct>100)return toast('掉落率必須大於 0 且不超過 100%');
    if(!Number.isFinite(qty)||qty<1||qty>9999||!Number.isInteger(qty))return toast('數量必須是 1～9999 的整數');
    beast.extra_drops=normalizeBeastExtraDrops(beast);if(beast.extra_drops.some(x=>x.item_id===id))return toast('此神獸已設定該額外掉落');
    beast.extra_drops.push({item_id:id,item_name:itemName(id),drop_chance:Math.round(pct*10000)/1000000,quantity:qty,enabled:true});renderEditor();toast('已加入 '+itemName(id)+'，請按「儲存額外掉落」');
  }
  function removeDivineBeastDrop(key,index){const beast=currentBeastByKey(key);if(!beast)return;beast.extra_drops=normalizeBeastExtraDrops(beast);beast.extra_drops.splice(index,1);renderEditor()}
  function setDivineBeastDropRate(key,index,value){const beast=currentBeastByKey(key),pct=Number(value);if(!beast)return;if(!Number.isFinite(pct)||pct<=0||pct>100){toast('掉落率必須大於 0 且不超過 100%');renderEditor();return}beast.extra_drops=normalizeBeastExtraDrops(beast);if(beast.extra_drops[index])beast.extra_drops[index].drop_chance=Math.round(pct*10000)/1000000}
  function setDivineBeastDropQuantity(key,index,value){const beast=currentBeastByKey(key),qty=Number(value);if(!beast)return;if(!Number.isFinite(qty)||qty<1||qty>9999||!Number.isInteger(qty)){toast('數量必須是 1～9999 的整數');renderEditor();return}beast.extra_drops=normalizeBeastExtraDrops(beast);if(beast.extra_drops[index])beast.extra_drops[index].quantity=qty}
  async function saveDivineBeastExtraDrops(key){
    const beast=currentBeastByKey(key);if(!beast)return toast('找不到神獸資料');
    const drops=normalizeBeastExtraDrops(beast).map(x=>({item_id:x.item_id,drop_chance:x.drop_chance,quantity:x.quantity,enabled:true}));
    try{const {data,error}=await sb.rpc('admin_divine_beast_replace_extra_drops',{p_beast_key:key,p_drops:drops});if(error)throw error;beast.extra_drops=clone(data||[]);toast('神獸額外掉落已儲存');addLog('更新神獸 '+key+' 額外掉落');renderEditor()}
    catch(e){console.error('[V15.4 PHASE1 FIX6] divine beast drops save failed',e);toast('神獸額外掉落儲存失敗：'+(e.message||e))}
  }
  ...
  window.filterItemValuesV154=filterItemValues;window.filterItemCategoryV154=filterItemCategory;window.selectMonsterV154=selectMonster;window.saveDivineBeastStatsV154=saveDivineBeastStats;window.filterMonsterSelectV154=filterMonsterSelect;window.filterMonsterDropItemsV154=filterMonsterDropItems;window.addMonsterDropV154=addMonsterDrop;window.removeMonsterDropV154=removeMonsterDrop;window.setMonsterDropRateV154=setMonsterDropRate;window.filterDivineBeastDropItemsV154=filterDivineBeastDropItems;window.addDivineBeastDropV154=addDivineBeastDrop;window.removeDivineBeastDropV154=removeDivineBeastDrop;window.setDivineBeastDropRateV154=setDivineBeastDropRate;window.setDivineBeastDropQuantityV154=setDivineBeastDropQuantity;window.saveDivineBeastExtraDropsV154=saveDivineBeastExtraDrops;
  ...
  console.info('[V15.4 ADMIN BALANCE PHASE1 FIX6 DIVINE BEAST EXTRA DROPS] installed',ITEM_EDITOR_BUILD);
